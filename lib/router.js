    ...
    return next().then(() => {
      ...
      if (!ctx.status || ctx.status === 404) {
        ...
        if (!~implemented.indexOf(ctx.method)) {
          if (options.throw) {
            const notImplementedThrowable = (typeof options.notImplemented === 'function')
          ...
          if (ctx.method === 'OPTIONS') {
            ...
            if (options.throw) {
              const notAllowedThrowable = (typeof options.methodNotAllowed === 'function')
...
Router.prototype.register = function(path, methods, middleware, opts) {
  ...
  if (Array.isArray(path)) {
    for (let i = 0; i < path.length; i++) {
      ...
      router.register(curPath, methods, middleware, opts);
    }
