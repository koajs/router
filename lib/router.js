...
for (const method_ of methods) {
  function setMethodVerb(method) {
    Router.prototype[method] = function (
      ...
      if (
        typeof pathOrMiddlewareOrValidation === 'string' ||
        pathOrMiddlewareOrValidation instanceof RegExp
      ) {
        ...
        middleware = [
          this.createValidationMiddleware(pathOrMiddlewareOrValidation),
...
Router.prototype.createValidationMiddleware = function (schema) {
  ...
  const validator = this.compiler.compile(bodySchema);
  ...
  return async (ctx, next) => {
    ...
    ctx.body = {
      ...
      message: validator.errors
        .map((err) => `body${err.instancePath} ${err.message}`)
        .join(', ')
    };
  };
};
