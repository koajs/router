...
const TypeCompiler = require('@sinclair/typebox/compiler').TypeCompiler;
...
function Router(opts = {}) {
  ...
  this.compiler = TypeCompiler.Compile;
}
...
for (const method_ of methods) {
  function setMethodVerb(method) {
    Router.prototype[method] = function (nameOrPath, pathOrMiddlewareOrValidation, optionalMiddleware) {
      let path;
      let name;
      let middleware;


      if (typeof pathOrMiddlewareOrValidation === 'string' || pathOrMiddlewareOrValidation instanceof RegExp) {
        middleware = Array.prototype.slice.call(arguments, 2);
        name = nameOrPath;
        path = pathOrMiddlewareOrValidation;
      } else if (typeof pathOrMiddlewareOrValidation === 'object') {
        middleware = [this.createValidationMiddleware(pathOrMiddlewareOrValidation), ...Array.prototype.slice.call(arguments, 2)];
        path = nameOrPath;
        name = null;
      } else {
...
Router.prototype.createValidationMiddleware = function (schema) {
  const { body: bodySchema } = schema;

  const validator = this.compiler(bodySchema);


  return async (ctx, next) => {
    const body = ctx.request.body;
    const result = validator.Check(body);


    if (!result) {
      const all = [...validator.Errors(body)]
      ctx.status = 400;
      ctx.body = {
        statusCode: 400,
        error: 'Bad request',
        message: all.map((err, i) => {
          return `body${err.path} ${err.message}`
        }).join(', ')
      };
    } else {
      await next();
